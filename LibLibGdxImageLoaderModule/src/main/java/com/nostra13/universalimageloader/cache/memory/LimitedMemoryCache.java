...
/**
 * Limited cache. Provides object storing. Size of all stored Pixmaps will not to exceed size limit (
...
public abstract class LimitedMemoryCache extends BaseMemoryCache {
	 ...
	 * time)
	 */
	private final List<Pixmap> hardCache = Collections.synchronizedList(new LinkedList<Pixmap>());
	...
	public boolean put(String key, Pixmap value) {
		...
		if (valueSize < sizeLimit) {
			while (curCacheSize + valueSize > sizeLimit) {
				Pixmap removedValue = removeNext();
	...
	protected abstract int getSize(Pixmap value);

	protected abstract Pixmap removeNext();
}
