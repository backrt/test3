...
public final class ImageSizeUtils {
    ...
    private static final int DEFAULT_MAX_Pixmap_DIMENSION = 2048;

    private static ImageSize maxImageSize;

    static {
        int[] maxPixmapSize = new int[1];
        GLES10.glGetIntegerv(GL10.GL_MAX_TEXTURE_SIZE, maxPixmapSize, 0);
        int maxPixmapDimension = Math.max(maxPixmapSize[0], DEFAULT_MAX_Pixmap_DIMENSION);
        maxImageSize = new ImageSize(maxPixmapDimension, maxPixmapDimension);
    }
    ...
    public static ImageSize defineTargetSizeForView(ImageAware imageAware, ImageSize maxImageSize) {
        float width = imageAware.getWidth();
        if (width <= 0) width = maxImageSize.getWidth();

        float height = imageAware.getHeight();
     ...
     * Pixmap. For example, inSampleSize == 4 returns an image that is 1/4 the width/height of the original, and 1/16
...
//        final float srcWidth = srcSize.getWidth();
//        final float srcHeight = srcSize.getHeight();
//        final float targetWidth = targetSize.getWidth();
//        final float targetHeight = targetSize.getHeight();
//
//        int scale = 1;
//
...
//            scale = 1;
//        }
//        scale = considerMaxPixmapSize(srcWidth, srcHeight, scale, powerOf2Scale);
//
//        return scale;
//    }

//    private static int considerMaxPixmapSize(int srcWidth, int srcHeight, int scale, boolean powerOf2) {
//        final int maxWidth = maxPixmapSize.getWidth();
//        final int maxHeight = maxPixmapSize.getHeight();
...
//    }
     ...
     * Pixmap size.<br />
     * We can't create Pixmap in memory with size exceed max Pixmap size (usually this is 2048x2048) so this method
...
//        final int targetWidth = maxPixmapSize.getWidth();
//        final int targetHeight = maxPixmapSize.getHeight();
//
